	...
	window.addEventListener('load', function () {
		...
		function get_sources(query) {
				...
				sources.push(
					{
						...
						templates: {
							...
							item({item, components, html}) {
								let value = '';
								switch (config['tmpl_suggestion']) {
									case 'autocomplete-post-suggestion':
										let img = '';
										if (typeof item.images === 'object' && !Array.isArray(item.images)) {
											img = html`<img class="suggestion-post-thumbnail" src="${item.images.medium.url}" alt="${item.post_title}" />`
										}
									...
									case 'autocomplete-term-suggestion':
										value = html`<div><a class="suggestion-link" href="${item.permalink}" title="${item.name}">
			...
			sources[last].templates.footer = function ({html}) {
				return html`<div class="autocomplete-footer">
